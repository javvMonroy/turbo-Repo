...
def gen_xml_v43(inv, sale_conditions, total_servicio_gravado,
    ...
    sb.Append('<Ubicacion>')
    sb.Append('<Provincia>' + (issuing_company.state_id.code or '1') + '</Provincia>')
    ...
    document_from_pos = inv.company_id.def_partner_id == inv.partner_id or False

    if inv.tipo_documento == 'TE' or (inv.tipo_documento == 'NC' and (inv.reversed_entry_id.tipo_documento == 'TE' or document_from_pos or (not receiver_company.vat and receiver_company.identification_id.code == '00'))):
        pass
    else:
        ...
        if receiver_company.name:
            ...
            if inv.tipo_documento == 'FEE':
                ...
                elif inv.tipo_documento == 'ND' and id_code == '05':
                    if receiver_company.vat:
                        sb.Append(
                            '<IdentificacionExtranjero>' + receiver_company.vat[:20] + '</IdentificacionExtranjero>')
                else:
    ...
    if inv.contingencia:
        sb.Append('<InformacionReferencia>')
        sb.Append('<TipoDoc>08</TipoDoc>')
        sb.Append("""<Numero>""" + inv.contingencia_number + """</Numero>""")
        sb.Append("""<FechaEmision>""" + str(inv.date.strftime('%Y-%m-%dT%H:%M:%S-06:00')) + """</FechaEmision>""")
        sb.Append('<Codigo>05</Codigo>')
        sb.Append('<Razon>Factura por contingencia</Razon>')
        sb.Append('</InformacionReferencia>')

    # Llenar datos de special tags lines en la factura
    if invoice_comments or inv.special_tags_lines:
        sb.Append('<Otros>')
        if invoice_comments:
            text = str(invoice_comments)
            text = text.replace('&', '&amp;')
            text = text.replace('<', '&lt;')
            text = text.replace('>', '&gt;')
            text = text.replace('"', '&quot;')
            text = text.replace("'", '&apos;')
            sb.Append('<OtroTexto>' + str(text) + '</OtroTexto>')
        for special_tag in inv.special_tags_lines:
            sb.Append("<%s codigo='%s'>%s</%s>" % (special_tag.element, special_tag.code, special_tag.content, special_tag.element))
        sb.Append('</Otros>')
